...
__all__ = ['drawWorld', 'demonstrateWorld', 'demonstrateTPPlacement',
           ...
           'drawGaussianOnTrial', 'drawGaussianScreen', 'makeImageArrayWithTools']
...
def makeImageArrayWithTools(toolpicker, path, worlddict, sample_ratio=1):
    world = loadFromDict(worlddict)
    #pg.init()
    def maketoolpt(p):
        return [int(p[0] + 45), int(45-p[1])]
    def drawSingleImg(world):
        s = pg.Surface((world.dims[0] + 150, world.dims[1]))
        s.fill(world.bk_col)
        s.blit(drawWorld(world), [0, 0])
        for i, t in enumerate(toolpicker._tools.keys()):
            col = COLORS[i]
            newsc = pg.Surface([96, 96])
            newsc.fill(col)
            toolsc = _draw_tool(toolpicker._tools[t], maketoolpt, [90,90])
            newsc.blit(toolsc, [3, 3])
            s.blit(newsc, (world.dims[0] + 30, 137 + 110*i))
        return s

    images = [drawSingleImg(world)]
    if len(path[(list(path.keys())[0])]) == 2:
        nsteps = len(path[list(path.keys())[0]][0])
    else:
        nsteps = len(path[list(path.keys())[0]])
        ...
        for onm, o in world.objects.items():
            if not o.isStatic():
                if len(path[onm])==2:
                    ...
                    o.setPos(path[onm][i][0:2])
                    o.setRot(path[onm][i][2])
        images.append(drawSingleImg(world))
    return images
